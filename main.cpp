...
string sanitize(const string& s) {
    ...
    for (char c : s) {
        ...
        if (uc > 127) { r += c; continue; }
        if (isalnum(uc)) { r += c; continue; }
        if (c == ' ' || c == '-' || c == '_' || c == '.') r += c;
    }
    return (r.empty() || r == "." || r == "..") ? "unnamed" : r;
}

string genFilename(const string& orig) {
    random_device rd; mt19937 gen(rd());
    auto t = duration_cast<seconds>(system_clock::now().time_since_epoch()).count();
    return to_string(t) + "_" + to_string(uniform_int_distribution<>(1000, 9999)(gen)) + fs::path(orig).extension().string();
}
...
void uploadMedia(const httplib::Request& req, httplib::Response& res) {
    log_message("Upload to: " + (req.has_param("album") ? req.get_param_value("album") : "N/A"));
    try {
        if (!auth(req)) { res.status = 401; res.set_content("Unauthorized", "text/plain"); return; }
        string album = sanitize(req.has_param("album") ? req.get_param_value("album") : req.path_params.at("album"));
        const string path = "uploads/" + album;
        if (!fs::exists(path)) { res.status = 404; res.set_content("Album not found", "text/plain"); return; }
        int uploaded = 0;
        ...
        log_message("Uploaded " + to_string(uploaded) + " to " + album);
        res.set_content(uploaded ? "Uploaded " + to_string(uploaded) : "No files", "text/plain");
    } catch (...) {
        log_message("Upload failed");
        res.status = 500; res.set_content("Upload failed", "text/plain");
    }
}
...
void serveGallery(const httplib::Request& req, httplib::Response& res) {
    log_message("Gallery: " + (req.path_params.count("event") ? url_decode(req.path_params.at("event")) : "N/A"));
    ...
    string event_from_url = req.path_params.count("event") ? url_decode(req.path_params.at("event")) : "";
    auto albums = listAlbums();
    string current_album = "";
    for (const auto& album : albums) {
        if (album == event_from_url) { current_album = album; break; }
    }
    tmpl.set("site_title", "Photo Gallery").set("current_album", current_album).setList("albums", albums);
    if (!current_album.empty()) tmpl.setList("photos", listMedia(current_album));
    res.set_content(tmpl.render(), "text/html");
}

void serveAdmin(const httplib::Request& req, httplib::Response& res) {
    log_message("Admin page");
...
void serveFile(const httplib::Request& req, httplib::Response& res) {
    log_message("File: " + req.path_params.at("filename") + " from " + req.path_params.at("event"));
    try {
        const string path = "uploads/" + sanitize(req.path_params.at("event")) + "/" + req.path_params.at("filename");
        if (!fs::exists(path)) { res.status = 404; res.set_content("Not found", "text/plain"); return; }
        ifstream f(path, ios::binary);
        res.set_content(string((istreambuf_iterator<char>(f)), {}), getMimeType(path));
    } catch (...) { res.status = 500; res.set_content("Error", "text/plain"); }
}

void serveThumb(const httplib::Request& req, httplib::Response& res) {
    try {
        // Just serve the original file - no resizing without libraries
        const string path = "uploads/" + sanitize(req.path_params.at("event")) + "/" + req.path_params.at("filename");
        if (!fs::exists(path)) { res.status = 404; res.set_content("Not found", "text/plain"); return; }
        ifstream f(path, ios::binary);
        res.set_content(string((istreambuf_iterator<char>(f)), {}), getMimeType(path));
...
void serveFavicon(const httplib::Request&, httplib::Response& res) {
    ...
    res.set_content(base64_decode(b64_favicon), "image/svg+xml");
}

int main() {
    auto startup_start_time = high_resolution_clock::now();
    loadPin();
    int port = 8080;
    ...
    svr.Get("/uploads/:event/thumbs/:filename", serveThumb);
    ...
    svr.Post("/api/stream_upload", [&](const httplib::Request &req, httplib::Response &res, const httplib::ContentReader &content_reader) {
        log_message("Stream upload: " + url_decode(req.get_header_value("X-Album")));
        if (!auth(req)) { res.status = 401; res.set_content("Unauthorized", "text/plain"); return; }
        if (!req.has_header("X-Album") || !req.has_header("X-Filename")) {
            res.status = 400; res.set_content("Missing Headers", "text/plain"); return;
        }
        ...
        if (req.has_header("X-Offset")) {
            try { offset = stoull(req.get_header_value("X-Offset")); } catch (...) {}
        }
        string dir_path = "uploads/" + album;
        if (!fs::exists(dir_path)) { res.status = 404; res.set_content("Album not found", "text/plain"); return; }
        string final_path = dir_path + "/" + filename;
        ios_base::openmode mode = ios::binary;
        mode |= (offset > 0) ? ios::app : ios::trunc;
        ofstream ofs(final_path, mode);
        if (!ofs) { res.status = 500; res.set_content("Cannot open file", "text/plain"); return; }
    ...
    auto startup_end_time = high_resolution_clock::now();
